...
import { CoinKlineStream } from '../models/coin-kline-stream';
...
export class BinanceKlineService {
  ...
  private coinSymbol : BehaviorSubject<string> = new BehaviorSubject<string>("BTC");
  ...
  public CoinUpdated: BehaviorSubject<CoinKlineStream> = new BehaviorSubject<
  CoinKlineStream
  >(new CoinKlineStream);
  ...
  constructor(private http: HttpClient) {
    ...
    console.log(this.coinSymbol.value);
    console.log(this.interval.value);
  }

  getData(): Observable<any> {
    return this.http
      .get(this.klineUrl+this.coinSymbol.value+"USDT"+this.klineQuery+this.interval.value)
      .pipe(
        map((data : any) => {
  ...
  getStockData(interval: string,symbol: string) {
    return this.http.get(`https://api.binance.com/api/v3/klines?symbol=${symbol}USDT&interval=${interval}`);
  }
}
